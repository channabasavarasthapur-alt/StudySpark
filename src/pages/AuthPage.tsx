...
export default function AuthPage({ mode }: AuthPageProps) {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground transition-colors duration-500">
      ...
      <main className="mx-auto grid min-h-[calc(100vh-80px)] max-w-7xl place-items-center px-4 py-10 sm:min-h-[calc(100vh-88px)] sm:px-6 sm:py-12">
        <section className="w-full max-w-md rounded-2xl bg-card p-6 shadow-sm sm:border sm:border-border sm:p-8">
          <div>
            ...
            <h1 className="mt-3 text-2xl font-black tracking-tight text-foreground sm:text-3xl">
              {isSignup ? 'Create your study workspace.' : 'Continue studying.'}
            </h1>
            <p className="mt-3 text-sm leading-6 text-muted">
              {isSignup
                ? 'Sign up, finish setup, and get your next study action.'
                : 'Log in to see what to study next.'}
            </p>
          </div>

          {!isConfigured && (
            <div className="mt-6 rounded-xl border border-red-500/15 bg-red-500/5 p-3 text-sm font-medium leading-6 text-red-500">
          ...
          <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
            <label className="block">
              ...
              <div className="mt-2 flex min-h-12 items-center gap-3 rounded-xl border border-border bg-background/70 px-4 py-3 focus-within:ring-2 focus-within:ring-purple/20">
            ...
            {errorMessage && (
              <div className="rounded-xl border border-red-500/15 bg-red-500/5 p-3 text-sm font-medium leading-6 text-red-500">
                {errorMessage}
              </div>
            )}

            {statusMessage && (
              <div className="rounded-xl border border-teal/15 bg-teal/5 p-3 text-sm font-medium leading-6 text-teal">
                {statusMessage}
              </div>
            )}
